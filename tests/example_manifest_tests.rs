    ...
    for line in manifest.lines().map(str::trim) {
        match line {
            ...
            value if in_example => {
                ...
                } else if let Some(parsed_path) =
                    parse_quoted_value(value, "path")
                {
...
fn push_example(
    ...
    match (name, path) {
        ...
        (name, path) => {
            panic!(
                "incomplete [[example]] entry: name={name:?}, path={path:?}"
            );
        }
    }
}
...
fn collect_rs_files(
    ...
    for entry in
        fs::read_dir(directory).expect("examples directory should be readable")
    {
