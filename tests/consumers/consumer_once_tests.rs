...
mod custom_consumer_once_tests {
    ...
    fn test_custom_consumer_into_box_chaining() {
        ...
        chained.accept(&5);
        assert_eq!(*log.lock().expect("mutex should not be poisoned"), vec![10, 105]);
    }
...
mod custom_consumer_to_methods_tests {
    ...
    fn test_custom_consumer_to_box_multiple_times() {
        ...
        boxed1.accept(&5);
        boxed2.accept(&6);
        assert_eq!(*log.lock().expect("mutex should not be poisoned"), vec![15, 18]);
    }
...
mod closure_to_xxx_methods_tests {
    ...
    fn test_closure_fnonce_into_fn() {
        ...
        let l = log.clone();
        let closure = move |x: &i32| {
            l.lock().expect("mutex should not be poisoned").push(*x + 100);
        };

        let func = closure.into_fn();
        func(&5);
        assert_eq!(*log.lock().expect("mutex should not be poisoned"), vec![105]);
    }
    ...
    fn test_closure_into_box_with_or_else() {
        ...
        conditional.accept(&-5);
        assert_eq!(*log.lock().expect("mutex should not be poisoned"), vec![5]); // or_else branch executed
    }
    ...
    fn test_closure_into_box_preserves_behavior() {
        ...
        assert_eq!(*log.lock().expect("mutex should not be poisoned"), vec![5, 5]);
    }
    ...
    fn test_complex_closure_capture() {
        ...
        let closure = move |x: &i32| {
            l.lock()
                .expect("mutex should not be poisoned")
                .push(*x * multiplier + addition);
        };

        let boxed = closure.into_box();
        boxed.accept(&3);

        assert_eq!(*log.lock().expect("mutex should not be poisoned"), vec![25]); // 3 * 5 + 10 = 25
    }
    ...
    fn test_closure_into_box_with_string() {
        ...
        let closure = move |s: &String| {
            l.lock().expect("mutex should not be poisoned").push(s.clone());
        };
...
mod advanced_to_methods_tests {
    ...
    fn test_to_fn_then_call() {
        ...
        let func = consumer.to_fn();
        func(&7);
        assert_eq!(*log.lock().expect("mutex should not be poisoned"), vec![7]);
    }
    ...
    fn test_to_box_preserves_original_for_reuse() {
        ...
        assert_eq!(*log.lock().expect("mutex should not be poisoned"), vec![1, 2, 3]);
    }
    ...
    fn test_default_trait_implementation_via_to_fn() {
        ...
        assert_eq!(*log.lock().expect("mutex should not be poisoned"), vec![18]);
    }
}
