...
mod closure_tests {
    ...
    fn test_closure_to_box_with_state() {
        ...
        impl LoggingConsumer {
            fn consume(&self, x: &i32, y: &i32) {
                self.log.lock().expect("mutex should not be poisoned").push(*x + *y);
            }
        }
        ...
        box_consumer2.accept(&10, &20);
        assert_eq!(*log.lock().expect("mutex should not be poisoned"), vec![8, 30]);
    }
    ...
    fn test_closure_to_fn_with_state() {
        ...
        func2(&10, &20);
        assert_eq!(*log.lock().expect("mutex should not be poisoned"), vec![15, 200]);
    }
}
...
mod default_implementation_tests {
    ...
    fn test_custom_into_box() {
        ...
        let boxed = consumer.into_box();
        boxed.accept(&5, &3);
        assert_eq!(*log.lock().expect("mutex should not be poisoned"), vec![16]); // (5 + 3) * 2 = 16
    }
    ...
    fn test_custom_into_fn() {
        ...
        let func = consumer.into_fn();
        func(&4, &2);
        assert_eq!(*log.lock().expect("mutex should not be poisoned"), vec![18]); // (4 + 2) * 3 = 18
    }
    ...
    fn test_custom_to_box() {
        ...
        boxed.accept(&3, &2);
        assert_eq!(*log.lock().expect("mutex should not be poisoned"), vec![20]); // (3 + 2) * 4 = 20

        // Verify original consumer is still usable (not consumed)
        consumer.accept(&1, &1);
        assert_eq!(*log.lock().expect("mutex should not be poisoned"), vec![20, 8]); // (1 + 1) * 4 = 8
    }
    ...
    fn test_custom_to_fn() {
        ...
        func(&2, &3);
        assert_eq!(*log.lock().expect("mutex should not be poisoned"), vec![25]); // (2 + 3) * 5 = 25

        // Verify original consumer is still usable (not consumed)
        consumer.accept(&1, &0);
        assert_eq!(*log.lock().expect("mutex should not be poisoned"), vec![25, 5]); // (1 + 0) * 5 = 5
    }
    ...
    fn test_custom_consumer_composition() {
        ...
        chained.accept(&5, &3);
        assert_eq!(*log.lock().expect("mutex should not be poisoned"), vec![16, 15]); // (5+3)*2=16, 5*3=15
    }
    ...
    fn test_custom_consumer_with_predicate() {
        ...
        conditional.accept(&5, &3);
        assert_eq!(*log.lock().expect("mutex should not be poisoned"), vec![80]); // (5+3)*10=80
        ...
        assert_eq!(*log2.lock().expect("mutex should not be poisoned"), vec![]); // Condition false, not executed
    }
    ...
    fn test_custom_consumer_multiple_clones() {
        ...
        assert_eq!(*log.lock().expect("mutex should not be poisoned"), vec![4, 8, 12]);
