...
Tester 是一个函数式抽象，用于判定状态或条件是否成立。它不接受
参数，返回布尔值，表示某个状态或条件的判定结果。
...
/// 无参条件判定器 - 判断状态或条件是否成立
///
/// Tester 封装了"判定某个状态或条件"的逻辑，不接受参数，返回布尔值。
/// 它通过闭包捕获需要判定的上下文信息。
pub trait Tester {
    /// 执行判定，返回判定结果
    ///
...
|------|------|------|----------|---------|---------|
| **Tester** | 无 | `bool` | `&self` | 判定状态/条件 | `Fn() -> bool` |
| **Predicate** | `&T` | `bool` | `&self` | 判断值 | `Fn(&T) -> bool` |
| **Supplier** | 无 | `T` | `&self` | 返回值 | `Fn() -> T` |
| **StatefulSupplier** | 无 | `T` | `&mut self` | 生成值 | `FnMut() -> T` |
| **Consumer** | `&T` | 无 | `&self` | 只读消费 | `Fn(&T)` |

**关键洞察**：
- **Tester 与 Predicate 类似**：都是"判断/判定"类抽象，使用 `&self`，
  不修改自身状态
- **Tester 与 Supplier 不同**：虽然都不接受输入，但 `Supplier`
  负责返回值，`StatefulSupplier` 还可能生成不同的值；而 Tester
  只负责读取状态并作出判定
- **状态管理是调用方的职责**：Tester 只负责"判定"，不负责"管理状态"
